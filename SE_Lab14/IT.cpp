...
namespace IT
{
	...
	void IdTable::PrintIdTable(const wchar_t* inFile)
	{
		...
		idStream << setw(15) << "Идентификатор:"
			<< setw(27) << "Тип данных:"
			<< setw(15) << "Значение:"
			<< setw(27) << "Длина строки:"
			<< setw(20) << "Первое вхождение:" << endl;
			...
			if (table[i].idtype == IT::IDTYPE::L)
			{
				...
				case IDDATATYPE::INT:
					idStream << "   " << this->table[i].id
						<< setw(17) << "INT"
						<< setw(15) << table[i].value.vint
						<< setw(27) << "-"
						<< setw(20) << table[i].firstApi << endl;
					break;

				case IDDATATYPE::STR:
					idStream << "   " << this->table[i].id
						<< setw(17) << "STR"
						<< setw(15) << table[i].value.vstr.str
						<< setw(27) << (int)table[i].value.vstr.len
						<< setw(20) << table[i].firstApi << endl;
					break;
				}
			}
		...
		idStream << setw(15) << "Идентификатор:"
			<< setw(26) << "Тип данных возврата:"
			<< setw(36) << "Количество переданных параметров:"
			<< setw(22) << "Первое вхождение:" << endl;
		...
		for (int i = 0; i < size; i++)
		{
			if (this->table[i].idtype == IT::IDTYPE::F)
			{
				...
				case IDDATATYPE::INT:
					idStream << "   " << table[i].id
						<< setw(26) << "INT"
						<< setw(36) << table[i].parmQuantity
						<< setw(22) << table[i].firstApi << endl;
					break;
		...
		idStream << setw(25) << "Имя родительского блока:"
			<< setw(20) << "Идентификатор:"
			<< setw(16) << "Тип данных:"
			<< setw(24) << "Тип идентификатора:"
			<< setw(21) << "Первое вхождение:" << endl;

		for (int i = 0; i < size; i++)
		{
			if (table[i].idtype == IT::IDTYPE::V || table[i].idtype == IT::IDTYPE::P)
			{
				string identifierType = (table[i].idtype == IT::IDTYPE::V) ? "V" : "P";

				switch (table[i].iddatatype)
				{
				case IDDATATYPE::INT:
					idStream << "   " << table[i].parrent_function
						<< setw(35 - strlen(table[i].parrent_function))
						<< table[i].id
						<< setw(20) << "INT"
						<< setw(15) << identifierType
						<< setw(20) << table[i].firstApi << endl;
					break;
